...
export async function uploadReport({
  ...
  if (!response.ok) {
    ...
    throw new Error(message || "Failed to upload report");
  }
...
export async function fetchOfficerQueue() {
  const response = await fetch(`${API_URL}/officer-queue`);
...
export async function confirmCase(
  ...
  const response = await fetch(`${API_URL}/confirm-case`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(message || "Failed to confirm case");
  }
...
export async function getAdvisory(reportId) {
  const response = await fetch(`${API_URL}/advisory/${reportId}`);
...
export async function fetchConfirmedCases() {
  const response = await fetch(`${API_URL}/confirmed-cases`);

  if (!response.ok) {
    throw new Error("Failed to fetch confirmed cases");
  }
